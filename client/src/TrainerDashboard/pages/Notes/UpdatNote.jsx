...
function UpdatNote() {
  ...
  const getNoteDetails = async(id)=>{
    try {
        let response = await axios.get(`http://localhost:4000/api/trainer/getnotesofID/${id}`);
  ...
  const handleSubmit = async(e)=>{
    ...
    try {
      ...
      let response = await axios.put(`http://localhost:4000/api/trainer/updatenotes/${id}`, formData);
  ...
  const handleSelectChange = (selectedValues) => {
    ...
    <div className='mb-3 rounded bg-slate-700' style={{border:"2px solid gray",padding:"2rem",textAlign:"center",margin:"auto"}}>
      ...
      <form onSubmit={handleSubmit}>
        ...
        <div>
          <button className="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded mt-5" type='submit'> Submit</button>
        </div>
      </form>
    </div>
  )
